...
readonly class DownloadService
{
    ...
    public function streamDownloadGeneration(array $results, string $question): void
    {
        // Étape 1: Préparation
        $this->sendStreamingEvent('download_step', ['message' => '📁 Préparation des fichiers...']);

        // Étape 2: Téléchargement S3
        $this->sendStreamingEvent('download_step', ['message' => '☁️ Téléchargement depuis S3...']);

        // Étape 3: Création ZIP
        $this->sendStreamingEvent('download_step', ['message' => '📦 Création de l\'archive ZIP...']);

        try {
            // ⭐ Appel de la génération réelle
            $downloadResult = $this->generateDownloadPackage($results, $question);

            if ($downloadResult['success']) {
                $stats = $downloadResult['stats'];

                // Succès: envoyer les infos de téléchargement
                $this->sendStreamingEvent('download_ready', [
                    'download' => [
                        'available' => true,
                        'status' => 'ready',
                ...
                // Erreur: envoyer le message d'erreur
                $this->sendStreamingEvent('download_error', [
                    'download' => [
                        'available' => false,
                        'status' => 'error',
